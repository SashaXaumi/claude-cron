...
- **Auto-provisioning is enabled.** The repo ships with `wrangler.toml`'s D1 binding declared *without* a `database_id`. Anyone modifying for local dev: run `wrangler dev` and Wrangler will create a local D1 automatically (it appears under `.wrangler/state/v3/d1`). To deploy production, either click the Deploy-to-Cloudflare button (which forks this repo and provisions D1 in the user's account) or run `wrangler deploy` from a clone — both work; both auto-provision the production D1 on first deploy and write the id back into the deployer's copy of `wrangler.toml`. Don't commit a populated `database_id` back to the template.
